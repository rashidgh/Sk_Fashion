...
const Electronic = () => {
  ...
  useEffect(() => {
    let fetchData = async () => {
      ...
      let arr = [];
      arr.push(data[8], data[9], data[10], data[11], data[12], data[13]);
      setState(arr);
    };
    fetchData();
  }, []);
          ...
          state.map((val, ind) => {
            return (
              <div key={ind} data-aos="flip-left">
                ...
                >
                  <button className={styles.electronicbuyNow} onClick={verify}>
                    <Link
                      style={{ color: "#fff" }}
                      to={
                        window.localStorage.getItem("userName")
                  ...
                  {window.localStorage.getItem("userName") ? (
                    store.filter(fg => fg.id === val.id).length === 1 ? (
                      <Link style={{ color: "#fff" }} to="/cart">
                        <button className={styles.electronicGotoCart}>
                          GO TO CART
                        </button>
                      </Link>
                    ) : (
                      ...
                      >
                        <button className={styles.electronicAddTocart}>
                          ADD TO CART
                        </button>
                      </Link>
                    )
